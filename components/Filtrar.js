...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, Dimensions, Alert, Switch } from 'react-native';
...
const Filtrar = ({ navigation }) => {
  ...
  const handleToggleUrgentes = () => setUrgentes(previousState => !previousState);
  ...
  const applyFilters = () => {
    Alert.alert('Filtrar', 'Filtragem aplicada');
    navigation.goBack();
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.backgroundColor }]}>
      ...
      <View style={styles.row}>
        <Text style={[styles.label, { color: colors.textColor }]}>Ver apenas urgentes</Text>
        <Switch
          value={urgentes}
          onValueChange={handleToggleUrgentes}
          thumbColor={urgentes ? '#4682B4' : '#f4f3f4'}
...
const styles = StyleSheet.create({
  ...
  row: {
    ...
    alignItems: 'center',
    marginBottom: 20,
    marginTop: 80,  // Adjust to place the content below the back button
  },
  label: {
    fontSize: 18,
    marginRight: 10,
